# students/signals.py (FINAL CORRECTED FILE - Imports Fixed)
...
from django.db.models import Prefetch  # ⬅️ FIX: ADDED THIS IMPORT
...
from .models import (
    ...
    ClinicVisit,
    BehaviorRecord,
    SectionEnrollment  # ⬅️ FIX: ADDED THIS IMPORT
)
...
# ======================================================
# ⭐️ DASHBOARD HELPER FUNCTIONS ⭐️
# ======================================================
...
# ======================================================
# ⭐️⭐️⭐️ THIS IS THE FIX (FOR THE INFINITE LOOP) ⭐️⭐️⭐️
# ======================================================
@receiver(post_save, sender=Enrollment)
def enrollment_post_save(sender, instance: Enrollment, created, **kwargs):
    """
    This signal fires AFTER an Enrollment (grade) is saved.
    The Enrollment model's save() method has ALREADY calculated its own 'pre_final'.
    
    This signal's ONLY jobs are:
    1. Recalculate the Student's overall General Average.
    2. Send a WebSocket notification.
    
    It does NOT call instance.save() again, which prevents the infinite loop.
    """
    print(f"Signal: post_save received for Enrollment ID {instance.id}")
    student_id = instance.student_id
    academic_year = instance.teacher_class.academic_year 

    # 1. Recalculate the student's general average
    # This is SAFE because it saves the Student model, not the Enrollment model
    updated_general_average = calculate_and_update_student_average(student_id, academic_year)

    # 2. Send the WebSocket notification
    channel_layer = get_channel_layer()
...
def student_changed(sender, instance: Student, created, **kwargs):
    ...
    try:
        # ⭐️ FIX: Updated query to use the new relationship ⭐️
        full_instance = Student.objects.prefetch_related(
            Prefetch(
                'section_enrollments',
                queryset=SectionEnrollment.objects.filter(is_active=True).select_related('section'),
                to_attr='active_enrollment' # Use the same to_attr as in StudentSerializer
            ),
            "attendance_records",
...
def behavior_record_post_save(sender, instance: BehaviorRecord, created, **kwargs):
    ...
    try:
        # ⭐️ FIX: Updated query to use the new relationship ⭐️
        instance_with_student = BehaviorRecord.objects.prefetch_related(
             Prefetch(
...
def clinic_visit_post_save(sender, instance: ClinicVisit, created, **kwargs):
    ...
    try:
        # ⭐️ FIX: Updated query to use the new relationship ⭐️
        instance_with_student = ClinicVisit.objects.prefetch_related(
             Prefetch(
                'student__section_enrollments',
                queryset=SectionEnrollment.objects.filter(is_active=True).select_related('section'),
                to_attr='current_enrollment_prefetch'
