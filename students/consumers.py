...
from django.db.models import Q, Prefetch # ⭐️ FIX: Added Prefetch

# ⭐️ FIX: Added SectionEnrollment
...
class ReportConsumer(AsyncWebsocketConsumer):
    ...
    async def receive(self, text_data):
        try:
            ...
            # ⭐️ FIX: Corrected typo in error message
            await self.send(text_data=json.dumps({"status": "error", "message": "A server error occurred."}))
    ...
    def search_students(self, search, grade, section):
        ...
        if search:
            query.add(
                Q(lrn__icontains=search) |  # ⭐️ FIX: Use 'lrn'
        ...
        if grade and grade != "all":
            # ⭐️ FIX: Query the related SectionEnrollment
            query.add(Q(section_enrollments__section__grade=grade) & Q(section_enrollments__is_active=True), Q.AND)
        
        # Add section filter if provided and not "all"
        if section and section != "all":
            # ⭐️ FIX: Query the related SectionEnrollment
            query.add(Q(section_enrollments__section__name__iexact=section) & Q(section_enrollments__is_active=True), Q.AND)
        ...
        # ⭐️ FIX: Run the query with Prefetch to get the active enrollment
        qs = Student.objects.filter(query).prefetch_related(
            Prefetch(
                'section_enrollments',
                queryset=SectionEnrollment.objects.filter(is_active=True).select_related('section'),
                to_attr='active_enrollment'
            )
        ).distinct().order_by('last_name', 'first_name')[:10]

        # Manually build the results
        results = []
        for s in qs:
            # ⭐️ FIX: Get grade/section from the prefetched active_enrollment
            active_enrollment = s.active_enrollment[0] if s.active_enrollment else None
            
            results.append({
                "id": s.id,
                "student_id": s.lrn, # ⭐️ FIX: Use 'lrn'
                "name": f"{s.last_name}, {s.first_name} {s.middle_name or ''}".strip(),
                "grade": active_enrollment.section.grade if active_enrollment else "N/A", # ⭐️ FIX
                "section": active_enrollment.section.name if active_enrollment else "N/A" # ⭐️ FIX
            })
        return results
